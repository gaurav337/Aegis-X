...
def build_phi3_prompt(
    ...
    # CLIP Adapter
    if "run_clip_adapter" in tool_results:
        r = tool_results["run_clip_adapter"]
        prompt_parts.append(f"🧠 CLIP Adapter: score={r.score:.3f}")
        if r.details.get("flagged_patches"):
            prompt_parts.append(f"  → Flagged regions: {', '.join(r.details['flagged_patches'])}")
    ...
    prompt_parts.append("2. Explain conflicts between tools (e.g., 'rPPG detected pulse but CLIP flagged texture...')")
    prompt_parts.append("3. Use probabilistic language ('suggests', 'consistent with') — avoid absolute certainty")
    prompt_parts.append("4. If C2PA verified, state 'Cryptographic provenance confirms authenticity' and skip other analysis")
    prompt_parts.append("5. Keep explanation under 150 words")
    
    # ── OUTPUT FORMAT ──
    prompt_parts.append("\n=== OUTPUT FORMAT ===")
    prompt_parts.append("Return JSON: {\"verdict\": \"REAL|FAKE|INCONCLUSIVE\", \"confidence\": 0.0-1.0, \"explanation\": \"...\"}")
