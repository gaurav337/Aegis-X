...
from core.tools.registry import registry, default_tool_registry
...
class ForensicAgent:
    ...
    def _run_cpu_phase(self, preprocess_result: Any) -> Generator[AgentEvent, Any, bool]:
        """
    ...
    def _run_gpu_phase(self, preprocess_result: Any) -> Generator[AgentEvent, Any, None]:
        ...
        for tool_name in gpu_tools:
            ...
            if tool_name == "run_sbi":
                clip_result = self.ensemble.tool_results.get("run_clip_adapter")
                if clip_result and clip_result.score > 0.70:
                    logger.debug("Skipping SBI (CLIP score > 0.70 = fully synthetic)")
                    continue
            ...
            decision = self.esc.evaluate(
                ...
                viable_pending_tools=viable_pending,
    ...
    def analyze(self, preprocess_result: Any) -> Generator[AgentEvent, Any, dict]:
        """
        ...
        # ── CPU PHASE ──
        for event in self._run_cpu_phase(preprocess_result):
            yield event
