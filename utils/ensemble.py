...
from utils.thresholds import (
    # Tool weights
    WEIGHT_SIGLIP, WEIGHT_SBI, WEIGHT_FREQNET,
...
def _extract_context(tool_results: List[ToolResult]) -> Dict[str, float]:
    ...
    for result in tool_results:
        ...
        elif tool_name in ["run_siglip_adapter", "run_clip_adapter"]:
...
def _route(result: ToolResult, context: Dict[str, float],
    ...
    # SigLIP
    if tool_name == "run_siglip_adapter":
...
def calculate_ensemble_score(
    ...
    if c2pa_result and c2pa_result.success and _safe_get_details(c2pa_result, "c2pa_verified", False):
        visual_tools_run = [
            ...
            if _normalize_tool_name(r.tool_name) in ["run_siglip_adapter", "run_sbi", "run_freqnet"]
    ...
    ensemble_score = round(max(0.0, min(1.0, total_contribution / total_weight)), 4)
